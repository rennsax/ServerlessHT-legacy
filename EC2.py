...
def step_worker(group_id):
    ...
    while True:
        ...
        while not step_request_queue[group_id].empty():
            request_data = step_request_queue[group_id].get()     # Get the step request data from the step request queue
            ...
            if current_number[group_id] == groups_number[group_id]: 
                ...
                response_data = {
                    'model_weights' : step_average_all,
                    'restart' : 1
                }
...
def main():
    ...
    for group_id in range(int(total_groups)):
        ...
        for i in range(int(num_parts)):
            ...
            lambda_client.invoke(FunctionName='Hyperparameter_optimization_group_rfm',
                                InvocationType='Event',
                                Payload=json.dumps(payload))
