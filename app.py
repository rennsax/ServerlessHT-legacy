...
def handler(event, context):
    ...
    time_thread = threading.Thread(target=time_watcher, args=(event, context.function_name, lock))
...
def train_model(event, context, lock):
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=input_lr, momentum=input_momentum)
    last_train_info = ""
    ...
    for epoch in range(epochs_done, total_epoch):
        ...
        last_train_info = f"Epoch: {epoch+1}, Batch: {i+1}, Loss: {running_loss/100:.3f}"
